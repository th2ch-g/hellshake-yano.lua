...
| `single_char_keys`              | 配列        | ASDFGHJKLNM0-9  | 1文字ヒント用キー（記号対応）                     |
...
| `use_numeric_multi_char_hints`  | 真偽値      | v:false         | 数字2文字ヒント（01-99, 00）を有効化              |
...
- **記号対応**: single_char_keysで記号（`;`, `:`, `[`, `]`, `'`, `"`, `,`, `.`, `/`, `\`, `-`, `=`, `` ` ``）を使用可能
- **数字2文字ヒント**: 大量の単語に対応する数字ヒント（01-99, 00）をオプションで有効化
...
`single_char_keys`で記号を使用して、より多くのヒントオプションを追加できます：
...
**メリット**:
- すべてのアルファベットキーを使わずに1文字ヒントの容量を増やせる
- 2文字組み合わせ用に複数文字キーを確保できる
- QWERTYキーボードでより人間工学的なキー配置

#### 数字2文字ヒント（新機能）

大量の単語を扱うための数字2文字ヒント（01-99, 00）を有効化：
...
**動作の仕組み**:
1. 1文字ヒント: A, S, D, F, G, H, J, K, L（9個のヒント）
2. アルファベット2文字ヒント: BB, BC, BE, CB, CC...（multi_char_keysの組み合わせ）
3. 数字2文字ヒント: 01, 02, 03, ..., 99, 00（最大100個のヒント）
...
hellshake-yano.vim v4.0は、積極的なコード削減とTDD駆動のリファクタリングにより、大幅に最適化されたアーキテクチャを導入し、100%の後方互換性を維持しながら**51.9%のコード削減**（16,994行 → 8,816行）を達成しました。
...
| **core.ts** | 4,699 | 2,702 | 42.5%削減 |
| **word.ts** | 4,923 | 1,838 | 62.7%削減 |
| **hint.ts** | 1,926 | 545 | 71.7%削減 |
| **削除ファイル** | - | 5ファイル (1,496行) | 完全削除 |
...
- **display.ts** - 表示・レンダリングユーティリティ
- **validation.ts** - 入力バリデーションロジック
- **validation-utils.ts** - バリデーションユーティリティ関数
- **dictionary.ts** - 辞書管理
...
- **word-char-utils.ts** - 文字種ユーティリティ
- **word-detector-base.ts** - 基底検出クラス
...
最適化プロセスで以下のファイルが完全に削除されました：

- `word/word-dictionary.ts`（742行）- dictionary.tsに統合
- `word/word-encoding.ts`（139行）- word.tsに統合
- `hint/hint-display.ts`（256行）- hint.tsに統合
